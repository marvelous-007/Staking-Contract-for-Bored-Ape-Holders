...
async function main() {
  ...
  const stakingAdd = staking.address;
  console.log(`STAKING CONTRACT ADDRESS IS ${stakingAdd}`);

  const mint = ethers.utils.parseEther("1000");
  const Token = await ethers.getContractFactory("rewardToken");
  const token = await Token.deploy("Tank", "TNK", mint, stakingAdd);
  ...
  const setRewardToken = await staking.setTokenAddress(TokenAddress);
    ...
    const approve = Usdc.connect(impersonatedSigner).approve(
      stakingAdd,
      mint
    );
    const approve2 = Usdc.connect(impersonatedSigner2).approve(
      stakingAdd,
      mint
    );
    const approve3 = Usdc.connect(impersonatedSigner3).approve(
      stakingAdd,
      mint
    );
    ...
    const contractBalance = await Usdc.balanceOf(stakingAdd);
    console.log(` CONTRACT USDC BALANCE IS ${contractBalance}`);

    const updateReward = await staking
    ...
    const updateReward3 = await staking
      .connect(impersonatedSigner)
      .updateReward();
    ...
    const ClaimReward = await staking
      .connect(impersonatedSigner)
      .claimReward(90000000);
    console.log(`Reward withdrawal completed..................`);

    const updateReward2 = await staking
      .connect(impersonatedSigner)
      .updateReward();
    ...
    const withdraw = await staking
      .connect(impersonatedSigner)
      .withdraw(1000000000);
    console.log(`Stake withdrawal completed..................`);

    const contractBalance2 = await Usdc.balanceOf(stakingAdd);
